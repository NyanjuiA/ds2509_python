...
def process_file():
   ...
   Returns:
       None
   """
   file_name = entry.get() # Get the file name from the text entry
   result_label.config(text="") # Clear the previous result
   ...
   try:
      ...
      # Handle the error when the program doesn't have access rights to the file
      ...
      # Handle other file related errors/issues (e.g., permission errors)
      result_label.config(text=f"Error: File access error:\n{os_error}")

   # Handle all other unexpected errors/exceptions
   except:
      result_label.config(text=f"Unexpected error occured. Please try again.")
      # in a full-fledged app, you can log the errors to file here.
...
result_label = tk.Label(root, text='',wraplength=450)#.pack(pady=10)
